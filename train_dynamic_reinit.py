...
from torchtitan.models.llama.Dropped_model_init import DroppedTransformer
...
def init_model(job_config, train_spec, tokenizer, parallel_dims, pp_mesh, device, world_mesh, float8_handler, model_state_dict=None, optimizer_state_dict=None, drop_list=None):
    ...
    last_drop_list_dict = None
    with torch.device("meta"):
        if drop_list:
            model_config.drop_list = drop_list
            model_config.need_init_weights = False
            model = model_cls.from_model_args(model_config)
        else:
            last_steps = find_max_step(f"{job_config.job.dump_folder}/{job_config.checkpoint.folder}")
            drop_list_path = f"{job_config.job.dump_folder}/{job_config.checkpoint.folder}/step-{str(last_steps)}/drop_list.json"  
            if os.path.exists(drop_list_path):
                with open(drop_list_path, "r") as f:
                    last_drop_list_dict = json.load(f)
            
                model_config.drop_list = last_drop_list_dict["drop_list"]
            model = model_cls.from_model_args(model_config)
    ...
    pp_schedule, has_first_stage, has_last_stage = None, None, None
    ...
    if optimizer_state_dict:
        optimizers.optimizers[0].load_state_dict(optimizer_state_dict)



    return model, optimizers, num_flop_per_token, pp_schedule, model_parts, has_first_stage, has_last_stage, last_drop_list_dict
...
def main(job_config: JobConfig):
    ...
    # drop_list = ['*#', '#', '*#', '#', '*#', '#', '*#', '#', '*#', '#', '*#', '#', '*#', '#', '*#', '#', '*#', '#', '*#', '#', '*#', '#', '*#', '#', '*#', '#', '*#', '#']
    # drop_list = ['#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#']
    model, optimizers, num_flop_per_token, pp_schedule, model_parts, has_first_stage, has_last_stage, last_drop_list_dict = init_model(job_config, train_spec, tokenizer, parallel_dims, pp_mesh, device, world_mesh, float8_handler, model_state_dict=None, optimizer_state_dict=None, drop_list=None)
    ...
    if last_drop_list_dict:
        current_drop_list = last_drop_list_dict["drop_list"]

        logger.info(f"{color.green}drop_list @ step {train_state.step}: {current_drop_list} {color.reset}")
    else:
        current_drop_list = train_spec.config[job_config.model.flavor].drop_list
        logger.info(f"{color.green}drop_list @ step {train_state.step}: None; Start training! {color.reset}")
    ...
    with maybe_enable_profiling(
        ...
        while train_state.step < job_config.training.steps:
            ...
            if last_drop_list_dict and train_state.step % job_config.checkpoint.interval == 0:
                drop_list_path = f"{job_config.job.dump_folder}/{job_config.checkpoint.folder}/step-{str(train_state.step)}/drop_list.json"

                with open(drop_list_path, "w") as f:
                    json.dump(last_drop_list_dict, f, indent=1)
                target_drop_num = last_drop_list_dict["drop_attn_count"] + last_drop_list_dict["job_info"]["num_each"]
            else:
                target_drop_num = 0

            if target_drop_num <= job_config.dropping.num_max and train_state.step % job_config.dropping.drop_freq == 0 and isinstance(model, DynamicTransformer):
                with torch.no_grad():
                    ...
                    for attn_drop_idx in attn_top_indices:
                        current_drop_list[attn_drop_idx] = current_drop_list[attn_drop_idx].replace("*", "")

                    for mlp_drop_idx in mlp_top_indices:
                        current_drop_list[mlp_drop_idx] = current_drop_list[mlp_drop_idx].replace("#", "")

                    logger.info(f"drop_list @ step {train_state.step}: {current_drop_list}")
                    ...
                    if torch.distributed.get_rank() == 0:
                        ...
                        drop_list_dict = {
                            ...
                            "drop_round": train_state.step // job_config.dropping.drop_freq,
                            "drop_attn_count": drop_attn_count,
                            "drop_mlp_count": drop_mlp_count,
                            "sims_attn_sum": sims_attn_sum,
                            "sims_mlp_sum": sims_mlp_sum,
                            "attn_top_indices": attn_top_indices,
                            "mlp_top_indices": mlp_top_indices,
                            "drop_list": current_drop_list,
                            "job_info": {
                                "drop_freq": job_config.dropping.drop_freq,
                                "num_each": job_config.dropping.num_each,
                                "sim_threshold": job_config.dropping.sim_threshold,
                                "dataset": job_config.dropping.dataset,
                                "batch_size": job_config.dropping.batch_size,
                                "seq_len": job_config.dropping.seq_len,
                                "macro_steps": job_config.dropping.macro_steps,
                            }
                        }
                        ...
                        with open(drop_list_path, "w") as f:
                            json.dump(drop_list_dict, f, indent=1)
                    ...
                    # optimizer_state_dict = {"state": new_optimizer_state, "param_groups": param_groups}
                ...
                #             print(f"Shape: {obj.shape}, Dtype: {obj.dtype}, Device: {obj.device}, Memory: {obj.element_size() * obj.numel() / 1024 / 1024:.2f} MB")
                #     except Exception as e:
                #         pass  # 忽略错误对象
                ...
                # # model, optimizers, num_flop_per_token, pp_schedule, model_parts, has_first_stage, has_last_stage = init_model(job_config, train_spec, tokenizer, parallel_dims, pp_mesh, device, world_mesh, float8_handler, model_state_dict=model_state_dict, optimizer_state_dict=optimizer_state_dict, drop_list=current_drop_list)
