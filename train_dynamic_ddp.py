...
def main(job_config: JobConfig):
    ...
    def loss_fn(pred, labels, model):
        sum_params = sum(p.sum() for p in model.parameters())
    ...
    with maybe_enable_profiling(
        ...
        while train_state.step < job_config.training.steps:
            ...
            # if torch.distributed.get_rank() == 0 and train_state.step == 2:
            #     import debugpy
            #     try:
            #         debugpy.listen(8201)
            #         print("Waiting for debugger attach")
            ...
            for micro_step in range(job_config.training.gradient_accumulation_steps):
                ...
                if parallel_dims.pp_enabled:
                    ...
                    with train_context(optional_context_parallel_ctx):
                        ...
                        loss = loss_fn(pred, labels, model)
            ...
            # if torch.distributed.get_rank() ==  0 and (train_state.step == job_config.training.steps or (train_state.step != 0 and train_state.step % job_config.checkpoint.interval == 0)):
            #     torch.save(model.state_dict(), f'model_weights_{train_state.step}.pth')
            ...
            with torch.no_grad():
                if train_state.step % job_config.dropping.drop_freq == 0 and isinstance(model, DynamicTransformer):
                    ...
                    attn_len, mlp_len = length_tensor.tolist()

                    if torch.distributed.get_rank() != 0:
                        attn_top_indices_tensor = torch.empty(attn_len, dtype=torch.int32, device=device_type)
                        mlp_top_indices_tensor = torch.empty(mlp_len, dtype=torch.int32, device=device_type)
                    else:
                        attn_top_indices_tensor = torch.tensor(attn_top_indices, dtype=torch.int32, device=device_type)
                        mlp_top_indices_tensor = torch.tensor(mlp_top_indices, dtype=torch.int32, device=device_type)

                    torch.distributed.broadcast(attn_top_indices_tensor, src=0)
                    torch.distributed.broadcast(mlp_top_indices_tensor, src=0)


                    attn_top_indices = attn_top_indices_tensor.tolist()
                    mlp_top_indices = mlp_top_indices_tensor.tolist()
